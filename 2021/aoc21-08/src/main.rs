...
fn parse(contents:&str) -> Vec<DisplayLine> {
    contents.lines().into_iter().map(|x| {
        let mut parts = x.split("|");

        DisplayLine{
            signal: parts.next().unwrap().trim().split(" ").map(|x| x.to_string()).collect(),
            output: parts.next().unwrap().trim().split(" ").map(|x| x.to_string()).collect(),
        }
    }).collect()
}

fn part1(contents:&Vec<DisplayLine>) -> String {
    let count:usize = contents.iter().map(|x| x.output.iter().filter(|y| match y.len() {
        2 => true,
        4 => true,
        3 => true,
        7 => true,
...
fn get_codes(signal: &Vec<String>) -> [String; 10] {
    ...
    let bottom_left_and_bottom: String =
        result[8].chars().into_iter().filter(|x| !result[4].contains(*x) && !result[7].contains(*x)).collect();
...
fn part2(contents:&Vec<DisplayLine>) -> String {
    let sum:u32 = contents.iter().map(|x| {
